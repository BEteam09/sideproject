...
class JobPostingServiceImpl implements JobPostingService {
  ...
  public List<JobPosting> saveNewJobPostings(
    ...
    final List<JobPosting> newPostings =
        jobPostings.stream()
            .filter(
                t -> {
  ...
  private Map<String, Company> preloadCompanies(List<Company> companies) {
    return companyRepository
        .findAllByNameIn(companies.stream().map(Company::getName).toList())
        .stream()
  ...
  private List<String> fetchExistingTitlesInBatches(List<String> titles) {
    ...
    for (int i = 0; i < titles.size(); i += batchSize) {
      ...
      result.addAll(
          jobPostingRepository.findAllByTitleIn(batch).stream()
              .filter(e -> Objects.nonNull(e.getCompany()))
  ...
  private List<JobPosting> saveJobPostingInBatches(List<JobPosting> newPostings) {
    ...
    for (int i = 0; i < newPostings.size(); i += batchSize) {
      List<JobPosting> batch = newPostings.subList(i, Math.min(i + batchSize, newPostings.size()));
