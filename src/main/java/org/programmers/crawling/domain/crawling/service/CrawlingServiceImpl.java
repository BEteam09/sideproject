...
public class CrawlingServiceImpl implements CrawlingService {
    ...
    public static final String URL = "https://www.wanted.co.kr/wdlist?country=kr&job_sort=job.recommend_order&years=-1&locations=all";
    ...
    private static Company getCompany(WebDriver driver, String companyName, String companyUrl) {
        ...
        final String companySize = driver.findElements(
                By.className("CompanyTagList_CompanyTagList__tag__hb_2Y")).stream()
            .map(WebElement::getText).filter(text -> text.contains("명")).findFirst()
            .orElse(null); // "명"을 포함하는 텍스트만 필터링
        final String foundedDate = driver.findElement(By.tagName("time")).getAttribute("datetime");
        final String industry = driver.findElements(By.className(COMPANY_INFO_TAG)).get(0)
            .getText();
        return Company.builder().companySize(companySize).foundedDate(Year.parse(foundedDate))
            .name(companyName).city(city).homepageUrl(companyUrl).industry(industry).build();
    }

    private static JobPosting getPosting(String salary, String postTitle, String desc,
        String closingDate, JobType jobType, String minEx, String maxEx) {
        return JobPosting.builder().salary(Long.parseLong(salary.isEmpty() ? "0" : salary))
            .title(postTitle).description(desc).closingDate(closingDate.equals("상시채용") ? null
                : LocalDateTime.parse(closingDate + " 00:00:00",
                    DateTimeFormatter.ofPattern("yyyy.MM.dd HH:mm:ss"))).jobType(jobType)
            .minExperience(Long.parseLong(minEx.isEmpty() ? "0" : minEx))
            .maxExperience(Long.parseLong(maxEx.isEmpty() ? "0" : maxEx)).build();
    }
    ...
    public void crawlAndSaveJobPostings() {
        ...
        try {
            ...
            final WebDriverWait wait = new WebDriverWait(driver,
                Duration.ofSeconds(15)); // 찾는 요소가 나올때 까지 Wait 걸어줌.
            wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.className("JobCard_JobCard__Tb7pI")));
            ...
            for (WebElement element : elements) {
                try {
                    ...
                    wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.className(EXPERIENCE_LV_TAG)));
                    final String closingDate = driver.findElement(By.className(CLOSING_DATE_TAG))
                        .getText();
                    final String desc = driver.findElements(By.className(DESC_TAG)).get(1)
                        .getText(); // 주요 업무
                    final String exLv = driver.findElements(By.className(EXPERIENCE_LV_TAG)).get(1)
                        .getText();

                    wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.className(POST_TITLE_TAG))); // 노서치가 자주 발생해서 wait 걸어줌.
                    final String postTitle = driver.findElement(By.className(POST_TITLE_TAG))
                        .getText();
                    final JobType jobType =
                        postTitle.contains("계약직") ? JobType.TEMPORARY : JobType.PERMANENT;
                    ...
                    wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.className(COMPANY_NAME_TAG)));
                    final String descLink = driver.findElement(By.className(COMPANY_NAME_TAG))
                        .getAttribute("href");
                    final String companyName = driver.findElement(By.className(COMPANY_NAME_TAG))
                        .getText();
                    ...
                    wait.until(
                        ExpectedConditions.visibilityOfElementLocated(By.className(SALARY_TAG)));
                    final List<WebElement> salaryElements = driver.findElements(
                        By.className(SALARY_TAG));
                    log.debug("Salary Elements size : {}",
                        salaryElements.size()); // 에러가 자주 발생하여 로그로 남겨놓기.....
                    ...
                    try {
                        driver.findElement(By.className("CompanyInfo_CompanyInfo__ShowMore__FX21j"))
                            .click();
                        companyUrl = driver.findElement(
                                By.className("CompanyInfo_CompanyInfo__Modal__Contents__JBQGq"))
                    ...
                    final JobPosting posting = getPosting(salary, postTitle, desc, closingDate,
                        jobType, minEx, maxEx);
            ...
            // DB에 존재하는 회사 목록
            final Map<String, Company> preloadCompanies = companyService.findAllByNameIn(
